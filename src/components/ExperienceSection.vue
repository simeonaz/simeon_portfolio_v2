...
<template>
  ...
  >
    ...
    <div>
      <ol class="group/list">
        <li class="mb-12" v-for="(experience, index) in experiences" :key="index">
          <div
          ...
          >
            <div
              class="absolute -inset-x-4 -inset-y-4 z-0 hidden rounded-md transition motion-reduce:transition-none lg:-inset-x-6 lg:block lg:group-hover:bg-slate-900/50 lg:group-hover:shadow-[inset_0_1px_0_0_rgba(148,163,184,0.1)] lg:group-hover:drop-shadow-lg"
            ></div>
            <header
              class="z-10 mb-2 mt-1 text-xs font-semibold uppercase tracking-wide text-slate-500 sm:col-span-2"
              :aria-label="experience.year"
            >
              {{ experience.year }}
            </header>
      ...
      <div class="mt-12">
        <a
          ...
          href="https://drive.google.com/file/d/1bvW63Ggdg1U_4GH9w-rv476JhjdlgyFL/view?usp=sharing"
          target="_blank"
          rel="noreferrer noopener"
